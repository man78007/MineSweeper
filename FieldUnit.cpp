   ...
   for (int c=0; c<Cols; c++)
    ...
    void TField::OnRightClick(int r,int c)
    {
     ...
     // ShowMessage("R="+IntToStr(r)+" "+IntToStr(c));
      ...
      {
       ShowMessage("ПОБЕДА!!!\nОткрыты все "+IntToStr(OptionForm->HowMany)+" мин");
       Open();
      }
    }
  ...
  //И проверить на НЕОЖИДАННЫЙ БАБАХ
      if (Cells[r][c].HasMine) {
        ...
        for (int r=0; r<Rows; r++)
         for (int c=0; c<Cols; c++)
         {
          if (Cells[r][c].Marked) Marked++;
          if (!Cells[r][c].HasMine) continue;
          if (!Cells[r][c].Opened) Hidden++;
         };
        AnsiString S ="Сапёр ошибается только один раз!\n";
        S = S + "Отмечено "+IntToStr(Marked)+" мин\n";
        S = S + "Не найдено "+IntToStr(Hidden)+" мин\n";
        ShowMessage(S);
        Open();
        return;
      };
